...
7. **Every action item needs an actual action.** Don't just list what's happening — say what the reader can do. Usually that's "public comment open at the hearing" or an email address. A 2-minute email beats attending a 3-hour hearing — say so. Be specific (date, email address, item number). Frame as nudges, not demands: "if you want to weigh in" not "you should show up." **For ambiguous items, explain the trade-off and let the reader decide.** Whenever possible, embed the relevant source link directly in the action item — agenda page, legislation text, meeting page, signup page, comment form, or project page. The reader should be able to go from digest → source in one click.
...
21. **Embed links wherever they reduce friction.** Action items should usually link to the agenda, legislation, signup page, project page, or comment instructions. In narrative sections, add links sparingly but usefully — when naming a specific article, hearing, project, or official document, link it if the reader would plausibly want to click through. Prefer a few high-value links over link spam.
22. **Out-of-district items: check USER.md first.** If the user lists watched neighborhoods or districts outside their own, surface notable items from those areas — but at lower weight than in-district items. Think "FYI" not "action item." Items from unwatched districts only appear if they set citywide precedent. When in doubt, cut it.
23. **Every district report is self-contained.** A reader should never need a separate citywide report. After district-specific sections (311 pulse, evictions, permits, supervisor activity, local actions), include a `## Citywide` section covering: budget/policy headlines, housing pipeline highlights, ethics & money, journalism roundup, and upcoming meetings. Write this section district-neutral — no "your supervisor," no district-specific 311 data. A D5 report and a D9 report should have structurally similar citywide sections, tuned only where district context genuinely matters.
