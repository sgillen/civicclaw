...
PARCELS_API = "https://data.sfgov.org/resource/acdm-wktn.json"
...
# --- Address and district extraction from legislation text ---

SF_ADDRESS_RE = re.compile(
    r'\b(\d{1,5}[-–\d]*\s+'
    r'(?:(?:North|South|East|West|N\.|S\.|E\.|W\.)\s+)?'
    r'[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*'
    r'\s+(?:Street|St|Avenue|Ave|Boulevard|Blvd|Way|Drive|Dr|Court|Ct|'
    r'Place|Pl|Lane|Ln|Road|Rd|Terrace|Ter|Circle|Cir|Alley|Park))\b',
    re.IGNORECASE
)

PARCEL_RE = re.compile(
    r'(?:Assessor.s\s+)?(?:Parcel\s+)?Block\s+(?:No\.?\s*)?(\d+)[,\s]+Lot\s+(?:No\.?\s*)?(\w+)',
    re.IGNORECASE
)


def extract_addresses(text):
    """Extract street addresses from legislation text."""
    return list(set(SF_ADDRESS_RE.findall(text)))


def extract_parcels(text):
    """Extract (block, lot) tuples from legislation text."""
    results = []
    for block_raw, lot_raw in PARCEL_RE.findall(text):
        # Block is always 4 digits, lot can be 3 digits or 3 digits + letter suffix
        block = block_raw.zfill(4)
        # Lot: extract numeric part, zero-pad to 3, re-append any letter suffix
        lot_digits = re.match(r'(\d+)(.*)', lot_raw)
        if lot_digits:
            lot = lot_digits.group(1).zfill(3) + lot_digits.group(2).upper()
        else:
            lot = lot_raw
        results.append((block, lot))
    return results
...
def lookup_district_by_parcel(block, lot):
    ...
    try:
        ...
        data = json.loads(fetch(url, timeout=10))
        if data:
            raw = data[0].get("supervisor_district", "")
            return int(float(raw)) if raw else None
    except Exception:
        pass
    # Fallback: try just block (all lots in a block are usually same district)
    try:
        where = urllib.request.quote(f"block_num='{block}'")
        url = f"{PARCELS_API}?$select=supervisor_district&$where={where}&$limit=1"
        data = json.loads(fetch(url, timeout=10))
        if data:
            raw = data[0].get("supervisor_district", "")
...
def enrich_item_with_location(item_dict):
    """Add address, block/lot, and district info to an item dict from its title text."""
    title = item_dict.get("title", "")
    addresses = extract_addresses(title)
    parcels = extract_parcels(title)
    district = None

    # Try parcel lookup first (most reliable)
    for block, lot in parcels:
        district = lookup_district_by_parcel(block, lot)
        if district:
            break

    item_dict["addresses"] = addresses
    item_dict["parcels"] = [{"block": b, "lot": l} for b, l in parcels]
    item_dict["item_district"] = district
    return item_dict
...
def parse_grid_rows(html):
    """Parse the Legistar meeting grid into structured item dicts.

    The grid (rgMasterTable) has columns: File#, Version, Agenda#, Name, Type, Status, Title, ...
    Each row is a <tr> with class rgRow or rgAltRow. We extract <td> cells per row.
    """
    # Find grid rows
    row_re = re.compile(r'<tr[^>]*class="(?:rgRow|rgAltRow)"[^>]*>(.*?)</tr>', re.DOTALL)
    cell_re = re.compile(r'<td[^>]*>(.*?)</td>', re.DOTALL)
    link_re = re.compile(r'LegislationDetail\.aspx\?ID=(\d+)&(?:amp;)?GUID=([0-9A-Fa-f-]+)', re.IGNORECASE)
    ...
    for row_match in row_re.finditer(html):
        ...
        file_num = clean_html(cells[0]).strip()
        name = clean_html(cells[3]).strip()
        item_type = clean_html(cells[4]).strip()
        status = clean_html(cells[5]).strip()
        title = clean_html(cells[6]).strip()

        # Skip empty/header rows
        if not title or len(title) < 10:
            continue

        # Extract legislation detail URL from file# cell
        leg_link = link_re.search(cells[0])
        leg_url = None
        if leg_link:
            leg_url = f"{LEGISTAR_BASE}/LegislationDetail.aspx?ID={leg_link.group(1)}&GUID={leg_link.group(2)}"
...
def fetch_meeting_detail(url):
    ...
    # Try structured grid parsing first
    grid_items = parse_grid_rows(html)

    if grid_items:
        # Enrich with location data (addresses, parcels, district)
        for item in grid_items:
            enrich_item_with_location(item)
        # Legacy compat: build flat text list for existing consumers
        deduped = [item["title"] for item in grid_items]
    else:
        # Fallback: original heuristic parsing for pages with non-standard grids
...
def get_meetings_in_range(start, end, state, keywords, mode="weekly"):
    ...
    for stub in stubs:
        ...
        results.append({
            ...
            "grid_items": detail.get("grid_items", []),
...
def main():
    ...
    if daily_mode:
        ...
        if as_json:
            user_district = config.get("district")

            def tag_item(gi):
                """Tag item with district and whether it's in the user's district."""
                d = gi.get("item_district")
                return {
                    "file_number": gi.get("file_number"),
                    "name": gi.get("name"),
                    "type": gi.get("type"),
                    "status": gi.get("status"),
                    "title": gi.get("title"),
                    "legislation_url": gi.get("legislation_url"),
                    "addresses": gi.get("addresses", []),
                    "parcels": gi.get("parcels", []),
                    "item_district": d,
                    "in_district": d == user_district if d and user_district else None,
                }
            ...
            print(json.dumps({
                "upcoming": [{
                    ...
                    "items": [tag_item(gi) for gi in m.get("grid_items", [])],
                } for m in upcoming],
                "recap": [{
