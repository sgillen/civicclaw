#!/usr/bin/env python3
"""Generate CivicClaw site markdown + source manifests for public district pages.

Canonical publish target:
- docs/data/d{district}.md
- docs/data/d{district}-sources.json
- docs/data/site-refresh-summary.json

This script is intentionally simple and deterministic. It works from the
canonical repo layout after the civicclaw consolidation and treats all
11 district pages as first-class publish targets.
"""
...
def build_district_sources(district: int) -> dict:
    ...
    journalism = maybe_run_json("sf_journalism.py", ["--days", "7", "--json"], timeout=90) or []
...
def build_last_updated_summary(completed: list[int], failed: list[dict]) -> dict:
...
def main():
    ...
    write_json(DOCS_DATA_DIR / "site-refresh-summary.json", build_last_updated_summary(completed, failed))
